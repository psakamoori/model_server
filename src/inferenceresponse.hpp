...
public:
    ...
    Status addOutput(const std::string& name, OVMSDataType datatype, const size_t* shape, size_t dimCount);
    Status getOutput(const char* name, InferenceTensor** tensor);
    Status addParameter(const char* parameterName, OVMSDataType datatype, const void* data);
