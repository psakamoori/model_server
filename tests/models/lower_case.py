...
with tf.Session(graph=tf.Graph()) as sess:
    ...
    input = tf.placeholder(tf.string, [None, 1], name='input') # defines input shape
    const = tf.constant(1.0, name="const")
    output = tf.strings.lower(input, encoding='', name='output')  # defines network operation
